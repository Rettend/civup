...
The legacy `tournament_players` table and player-ID columns remain only for additive migrate-before-deploy compatibility. Migration `0026_tournament_entries.sql` backfills them into deterministic one-member entries. New tournament decisions use entry columns.
