  ...
  for (const [index, guildId] of orderedGuildIds.entries()) {
    ...
    try {
      if (action === 'sync') {
        ...
        if (result.pendingDiscordChanges === 0 && await getRankedRolesDirtyState(kv, guildId)) {
          await clearRankedRolesDirtyState(kv, guildId)
        }
        continue
      }
      ...
      console.error(`[maintenance] Ranked role ${action} failed for guild ${guildId}:`, error)
    }
  }
