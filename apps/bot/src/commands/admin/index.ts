...
export const command_admin = factory.command<AdminVar>(
  new Command('admin', 'Admin commands')
    ...
    .options(
      ...
      new SubGroup('tournament', 'Tournament management').options(
        ...
        new SubCommand('start', 'Start qualifier play for the current tournament'),
        new SubCommand('cut', 'Lock qualifiers and create playoff pairings'),
        new SubCommand('edit', 'Edit the current tournament settings'),
        new SubCommand('status', 'Show current tournament status'),
      ),
      new SubCommand('setup', 'View or configure server features').options(
        new Option('target', 'Feature to configure').choices(
          { name: 'Draft', value: 'draft' },
          { name: 'Archive', value: 'archive' },
          { name: 'Bot Commands', value: 'commands' },
          { name: 'Leaderboard', value: 'leaderboard' },
          { name: 'Civ Leaderboard (All)', value: 'civ-leaderboard-all' },
          ...
          { name: 'Browser Access', value: 'browser' },
        ),
        new Option('value', 'Value for features that require one').choices(
          { name: 'On', value: 'on' },
          { name: 'Off', value: 'off' },
        ),
      ),
  ...
  (c) => {
    ...
    if (c.sub.string === 'tournament start') return handleTournamentStart(c)
