...
interface InstalledLeaderDataInfo {
  versionLabel: string | null
  modifiedAt: number
}

interface LeaderDataMetaSnapshot {
  liveVersionLabel: string | null
  betaVersionLabel: string | null
  hasBetaLeaderData: boolean
}
...
async function main(): Promise<void> {
  ...
  await writeFile(LEADER_DATA_META_OUTPUT_PATH, renderLeaderDataMetaTs(leaderDataMeta), 'utf8')
...
async function readLeaderDataMeta(): Promise<LeaderDataMetaSnapshot> {
  const live = await readInstalledLeaderDataInfo('live')
  const beta = await readInstalledLeaderDataInfo('beta').catch(() => null)
  const hasBetaLeaderData = beta != null && beta.modifiedAt > live.modifiedAt
...
function resolveSourceVariant(variant: Variant, leaderDataMeta: LeaderDataMetaSnapshot): Variant {
  if (variant === 'beta' && !leaderDataMeta.hasBetaLeaderData) return 'live'
  return variant
}
