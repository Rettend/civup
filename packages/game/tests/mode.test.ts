...
describe('shared mode helpers', () => {
  ...
  test('uses centralized balance and unranked metadata', () => {
    ...
    expect(isUnrankedMode('6v6')).toBe(false)
  ...
  test('expands leaderboard tracks to game modes', () => {
    ...
    expect(leaderboardModesToGameModes('squad')).toEqual(['3v3', '4v4', '5v5', '6v6'])
    expect(leaderboardModesToGameModes('ffa')).toEqual(['ffa'])
    expect(leaderboardModesToGameModes('red-death')).toEqual(['ffa', '1v1', '2v2', '3v3', '4v4', '5v5', '6v6'])
  })
  ...
  test('uses strict mode player-count defaults', () => {
    expect(playerCountOptions('ffa')).toEqual([8, 12])
    ...
    expect(maxPlayerCount('ffa')).toBe(12)
    expect(maxPlayerCount('2v2')).toBe(8)
    expect(maxPlayerCount('5v5')).toBe(10)
    expect(maxPlayerCount('6v6')).toBe(12)
    expect(startPlayerCountOptions('ffa', 8)).toEqual([6, 7, 8])
    expect(startPlayerCountOptions('ffa', 12)).toEqual([6, 7, 8, 9, 10, 11, 12])
    expect(startPlayerCountOptions('ffa', 12, { permanentAlly: true })).toEqual([6, 8, 10, 12])
    ...
    expect(canStartWithPlayerCount('ffa', 8, 12)).toBe(true)
    expect(canStartWithPlayerCount('ffa', 8, 8)).toBe(true)
    expect(canStartWithPlayerCount('ffa', 6, 12)).toBe(true)
    expect(canStartWithPlayerCount('ffa', 10, 8)).toBe(false)
    expect(canStartWithPlayerCount('ffa', 10, 12)).toBe(true)
    expect(canStartWithPlayerCount('ffa', 12, 12)).toBe(true)
    expect(canStartWithPlayerCount('ffa', 7, 12)).toBe(true)
    expect(canStartWithPlayerCount('ffa', 7, 12, { permanentAlly: true })).toBe(false)
