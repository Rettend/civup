...
describe('shared mode helpers', () => {
  ...
  test('uses strict mode player-count defaults', () => {
    ...
    expect(startPlayerCountOptions('ffa', 8)).toEqual([6, 8])
    expect(startPlayerCountOptions('ffa', 12)).toEqual([6, 8, 10, 12])
    ...
    expect(canStartWithPlayerCount('ffa', 7, 12)).toBe(false)
